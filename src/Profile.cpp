...
/*
 ...
 ==================================
 */
Profile::Profile(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit, Gtk::Label *rulerlabel) :
      LagDisplay(config, bucketlimit), rulerlabel(rulerlabel), profps(0), drawpoints(true), drawmovingaverage(false), imageexists(false), slanted(true), slantwidth(5), showheightscale(false), totnumpoints(0), mavrgrange(5), linez(NULL), previewdetailmod(0.3), flightlinepoints(NULL), vertices(NULL), colours(NULL), samplemaxz(0), sampleminz(0), viewer(Point(0, 0, 0)), width(0), rulering(false), rulerwidth(3), classificationsHappening(0), isProcessingFence(false), fencing(false), hideProfNoise(false)
{
...
void Profile::resetview()
{
   ...
   // much room.
   glOrtho(-pixelsToImageUnits(get_width() / 2), +pixelsToImageUnits(get_width() / 2), -pixelsToImageUnits(get_height() / 2), +pixelsToImageUnits(get_height() / 2), -5 * width, +5 * width);
...
bool Profile::returntostart()
{
   ...
   centre.move((start.getX() + end.getX()) / 2, (start.getY() + end.getY()) / 2, (samplemaxz + sampleminz) / 2);
   zoomlevel = 1;
   double breadth = end.getX() - start.getX();
   double height = end.getY() - start.getY();
...
bool Profile::loadprofile(vector<double> profxs, vector<double> profys, int profps)
{
   ...
   if(!( // but skip this check if profxs and profys match current versions
         // AKA, true iff (profxs or profys is different to stored version)
   equal(this->profxs.begin(), this->profxs.end(), profxs.begin()) && equal(this->profys.begin(), this->profys.end(), profys.begin())) && (classificationsHappening > 0 || hasClassifyJobs()))
   {
      cout << "\b";
      return false;
   }
   ...
   end.move((profxs[profps - 1] + profxs[profps - 2]) / 2, (profys[profps - 1] + profys[profps - 2]) / 2, 0);
      ...
      if((*pointvector)[i]->getIncacheList()[0])
      {
         ...
         for(int j = 0; j < (*pointvector)[i]->getNumberOfPoints(0); j++)
         {
            ...
            if(correctpointsbuckets[i][j])
            {
               ...
               if(find(flightlinestot.begin(), flightlinestot.end(), (*pointvector)[i]->getPoint(j, 0).getFlightline()) == flightlinestot.end())
               {
                  // add it.
                  flightlinestot.push_back((*pointvector)[i]->getPoint(j, 0).getFlightline());
               }
            }
         }
      }
   ...
   for(int i = 0; i < (int) flightlinestot.size(); ++i)
   {
         ...
         if((*pointvector)[j]->getIncacheList()[0])
         {
            ...
            for(int k = 0; k < (*pointvector)[j]->getNumberOfPoints(0); k++)
            {
               ...
               if(correctpointsbuckets[j][k])
               {
                  ...
                  if((*pointvector)[j]->getPoint(k, 0).getFlightline() == flightlinestot[i])
                  {
                     //Add it
                     flightlinepoints[i].push_back((*pointvector)[j]->getPoint(k, 0));
      ...
      sort(flightlinepoints[i].begin(), flightlinepoints[i].end(), boost::bind(&Profile::linecomp, this, _1, _2));
   ...
   for(int i = 0; i < bucket->getNumberOfPoints(0); ++i)
   {
      ...
      if(correctpoints[i])
      {
         ...
         { //For every edge:
            ...
            if((zs[j] < bucket->getPoint(i, 0).getZ() && zs[lastcorner] >= bucket->getPoint(i, 0).getZ()) || (zs[lastcorner] < bucket->getPoint(i, 0).getZ() && zs[j] >= bucket->getPoint(i, 0).getZ()))
            {
               ...
               pnt.move(xs[j] + ((bucket->getPoint(i, 0).getZ() - zs[j]) / (zs[lastcorner] - zs[j])) * (xs[lastcorner] - xs[j]), ys[j] + ((bucket->getPoint(i, 0).getZ() - zs[j]) / (zs[lastcorner] - zs[j])) * (ys[lastcorner] - ys[j]), 0);
...
bool Profile::linecomp(LidarPoint a, LidarPoint b)
{
   ...
   else
   {
      ...
      double widgradboxa = multy * (ya - minPlan.getY()) - (multx * (xa - minPlan.getX()) * widgradbox);
      double widgradboxb = multy * (yb - minPlan.getY()) - (multx * (xb - minPlan.getX()) * widgradbox);
...
/*
 ...
 ==================================
 */
int Profile::get_closest_element_position(LidarPoint value, vector<LidarPoint>::iterator first, vector<LidarPoint>::iterator last)
{
   ...
   for(;;)
   {
      ...
      {
         first = middle;
      } else if(linecomp(value, *middle)) //ELSE IF the opposite, make the "last" point equal to the "middle" point
      {
         last = middle;
      }
...
bool Profile::on_pan(GdkEventMotion* event)
{
   if((event->state & Gdk::BUTTON1_MASK) == Gdk::BUTTON1_MASK || (event->state & Gdk::BUTTON2_MASK) == Gdk::BUTTON2_MASK)
   {
      ...
      centre.translate(-hypotenuse * breadth / length, -hypotenuse * height / length, pixelsToImageUnits(event->y - panStart.getY()));
...
bool Profile::on_pan_key(GdkEventKey *event, double scrollspeed)
{
   ...
   switch(event->keyval)
   {
      ...
      case GDK_s: // Down
         ...
         break;
      case GDK_a: // Left
         centre.translate(-hypotenuse * breadth / length, -hypotenuse * height / length, 0);
         return drawviewable(1);
         break;
      case GDK_d: // Right
         centre.translate(hypotenuse * breadth / length, hypotenuse * height / length, 0);
...
bool Profile::on_fence_start(GdkEventButton* event)
{
   if(event->button == 1)
   {
      ...
      activeFence.second.translate(hypotenuse * breadth / length, hypotenuse * height / length, -pixelsToImageUnits(event->y - get_height() / 2));
...
bool Profile::on_fence_key(GdkEventKey *event, double scrollspeed)
{
   ...
   switch(event->keyval)
   {
      ...
      case GDK_S: // Down
         ...
         break;
      case GDK_A: // Left
         activeFence.first.translate(-hypotenuse * breadth / length, -hypotenuse * height / length, 0);
         
         activeFence.second.translate(-hypotenuse * breadth / length, -hypotenuse * height / length, 0);
         break;
      case GDK_D: // Right
...
void Profile::makefencebox(Point fenceStart, Point fenceEnd, Colour c)
{
   ...
   if(slanted)
   {
      ...
      glVertex3d(fenceStart.getX() - (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(), fenceStart.getY() - (slantwidth / 2) * deltazratio * height / horiz - centre.getY(), fenceStart.getZ() + (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceStart.getX() + (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(), fenceStart.getY() + (slantwidth / 2) * deltazratio * height / horiz - centre.getY(), fenceStart.getZ() - (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceEnd.getX() + (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(), fenceEnd.getY() + (slantwidth / 2) * deltazratio * height / horiz - centre.getY(), fenceEnd.getZ() - (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceEnd.getX() - (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(), fenceEnd.getY() - (slantwidth / 2) * deltazratio * height / horiz - centre.getY(), fenceEnd.getZ() + (slantwidth / 2) * horizratio - centre.getZ());
      glEnd();
   } else
   {
      glBegin(GL_LINE_LOOP);
      glVertex3d(fenceStart.getX() - centre.getX(), fenceStart.getY() - centre.getY(), fenceStart.getZ() - centre.getZ());
      glVertex3d(fenceStart.getX() - centre.getX(), fenceStart.getY() - centre.getY(), fenceEnd.getZ() - centre.getZ());
      glVertex3d(fenceEnd.getX() - centre.getX(), fenceEnd.getY() - centre.getY(), fenceEnd.getZ() - centre.getZ());
      glVertex3d(fenceEnd.getX() - centre.getX(), fenceEnd.getY() - centre.getY(), fenceStart.getZ() - centre.getZ());
      glEnd();
   }
}
...
bool Profile::on_ruler_start(GdkEventButton* event)
{
   if(event->button == 1)
   {
      ...
      rulerEnd.move(centre.getX() + viewer.getX() + hypotenuse * breadth / length, centre.getY() + viewer.getY() + hypotenuse * height / length, centre.getZ() + viewer.getZ() - pixelsToImageUnits(event->y - get_height() / 2));
...
void Profile::makerulerbox()
{
   ...
   // Draw the Ruler
   glBegin(GL_LINES);
   glVertex3d(rulerStart.getX() - centre.getX(), rulerStart.getY() - centre.getY(), rulerStart.getZ() - centre.getZ());
   glVertex3d(rulerEnd.getX() - centre.getX(), rulerEnd.getY() - centre.getY(), rulerEnd.getZ() - centre.getZ());
   glEnd();
   glLineWidth(1);
}
...
void Profile::drawoverlays()
{
   ...
   // queued classification jobs
   {
      ...
      for(list<ClassificationJob>::iterator it = classificationQueue.begin(); it != classificationQueue.end(); it++)
         makefencebox(it->first.first, it->first.second, queuedFenceColour);
   }
   
   // active classification job
   if(isProcessingFence)
      makefencebox(processingFence.first, processingFence.second, processingFenceColour);
...
bool Profile::on_zoom(GdkEventScroll* event)
{
   ...
   centre.translate(hypotenuse * breadth / length, hypotenuse * height / length, -pixelsToImageUnits(event->y - get_height() / 2));
         ...
         for(int j = startindex; j <= endindex; j += detail)
         {
            ...
            switch(colourBy)
            {
               ...
               case colourByFlightline:
                  tempColour = getColourByFlightline(flightlinepoints[i][j].getFlightline());
                  break;
               case colourByClassification:
                  tempColour = getColourByClassification(flightlinepoints[i][j].getClassification());
                  break;
               case colourByReturn:
                  tempColour = getColourByReturn(flightlinepoints[i][j].getReturn());
                  break;
                  // ColourByNone
               default:
