...
/*
 ...
 ==================================
 */
FileOpener::FileOpener(TwoDeeOverview *tdo, Profile *prof, const Glib::RefPtr<Gtk::Builder>& builder, AdvancedOptionsWindow *aow, FileSaver *fs, int bucketlimit, Gtk::EventBox *eventboxtdo, Gtk::EventBox *eventboxprof, TwoDeeOverviewWindow *tdow, AdvancedLoadDialog* ald) :
      lidardata(NULL), tdo(tdo), prof(prof), tdow(tdow), aow(aow), fs(fs), ald(ald), eventboxtdo(eventboxtdo), eventboxprof(eventboxprof), bucketlimit(bucketlimit), loadworker(NULL), num_files_loading(0), newQuadtree(true), loadedanyfiles(false)
...
int FileOpener::on_quit()
{
   ...
   {
      std::cout << "Removing temporary file: " << LoadWorker::point_data_paths[i].c_str() << std::endl;
...
void FileOpener::on_filechooserdialogresponse(int response_id)
{
   ...
   } else if(response_id == 1 || response_id == 2)
   {
      ...
      // Create LoadWorker
      loadworker = new LoadWorker(this, pointskipselect->get_value_as_int(), names, create_new_quadtree, fenceusecheck->get_active(), resdepthselect->get_value_as_int(), resbaseselect->get_value_as_int(), bucketlevels, bucketlimit, cachelimit, btnUseDefault->get_active(), scale_factor, asciicodeentry->get_text(), fence, ald->get_point_filter(), cache_folder_select->get_current_folder());
...
void FileOpener::add_line()
{
   ...
   o << "Loading file " << ((numlines + 1) > num_files_loading ? num_files_loading : (numlines + 1)) << " of " << num_files_loading;
