...
/*
 ...
 ==================================
 */
TwoDeeOverviewWindow::TwoDeeOverviewWindow(TwoDeeOverview *tdo, AdvancedOptionsWindow *aow, FileSaver *fs, Gtk::Window *tdowin, Gtk::Window *profilewindow, const Glib::RefPtr<Gtk::Builder>& builder, Gtk::EventBox *eventboxtdo, ProfileWindow *profwin) :
      tdo(tdo), profwin(profwin), eventboxtdo(eventboxtdo), tdowin(tdowin), profilewindow(profilewindow), aow(aow), fs(fs), drawwhentoggled(true)
{
   load_xml(builder);
   ...
   showlegendcheck->set_inconsistent(!colourbyintensitymenu->get_active() && !colourbyheightmenu->get_active() && !colourbyclassificationmenu->get_active() && !colourbyreturnmenu->get_active());
}
...
void TwoDeeOverviewWindow::on_rulertoggleover()
{
   ...
   {
      tdo->unsetupruler();
      if(tdo->is_realized() && !profiletoggle->get_active() && !rulertoggleover->get_active() && !fencetoggle->get_active())
         tdo->drawviewable(2);
   }
}
...
void TwoDeeOverviewWindow::on_profiletoggle()
{
   ...
   {
      tdo->unsetupprofile();
      if(tdo->is_realized() && !profilewindow->get_visible())
