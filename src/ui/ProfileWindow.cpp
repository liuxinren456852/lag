...
/*
 ...
 ==================================
 */
ProfileWindow::ProfileWindow(Profile *prof, TwoDeeOverview *tdo, Gtk::Window *profilewindow, Gtk::Window *overviewwindow, Gtk::EventBox *eventboxprof, const Glib::RefPtr<Gtk::Builder>& builder, AdvancedOptionsWindow *aow) :
...
void ProfileWindow::on_fencetoggleprof()
{
   ...
   {
      prof->unsetupfence();
      if(prof->is_realized() && !rulertoggle->get_active() && !fencetoggleprof->get_active())
         prof->drawviewable(1);
   }
}
