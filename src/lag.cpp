...
string findgladepath(char* programpath)
{
   ...
   {
      std::cerr << "No lag.ui glade file found" << std::endl << "Tried: " << std::endl << gladename1 << std::endl << gladename2;
      exit(1);
   }
}
...
int main(int argc, char** argv)
{
   ...
   //The 2d overview.
   TwoDeeOverview *tdo = new TwoDeeOverview(glconfig, bucketlimit, rulerlabelover);
   ...
   ProfileWindow *profwin = new ProfileWindow(prof, tdo, profilewindow, overviewwindow, eventboxprof, builder, aow);
   
   //This contains the widgets of the 2D overview window.
   TwoDeeOverviewWindow *tdow = new TwoDeeOverviewWindow(tdo, aow, fs, overviewwindow, profilewindow, builder, eventboxtdo, profwin);
   ...
   FileOpener *fo = new FileOpener(tdo, prof, builder, aow, fs, bucketlimit, eventboxtdo, eventboxprof, tdow, ald);
