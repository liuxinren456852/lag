...
/*
 ...
 ==================================
 */
TwoDeeOverview::TwoDeeOverview(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit, Gtk::Label *rulerlabel) :
      LagDisplay(config, bucketlimit), drawnsofarminx(0), drawnsofarminy(0), drawnsofarmaxx(1), drawnsofarmaxy(1), resolutionbase(1), resolutiondepth(1), numbuckets(0), raiseline(false), linetoraise(0), drawnsinceload(false), reversez(false), showlegend(false), showdistancescale(false), drawneverything(false), pointcount(0), vertices(NULL), colours(NULL), tdoDisplayNoise(true), rulerlabel(rulerlabel), profiling(false), showprofile(false), draw_pointinfo(false), fencing(false), showfence(false), rulerwidth(2), rulering(false), heightenNonC(false), heightenGround(false), heightenLowVeg(false), heightenMedVeg(false), heightenHighVeg(false), heightenBuildings(false), heightenNoise(false), heightenMass(false), heightenWater(false), heightenOverlap(false), heightenUndefined(false), panningRefresh(1), slicing(false), latlong(false), superzoom(false)
{
            ...
            if(tdoDisplayNoise || (!tdoDisplayNoise && buckets[i]->getPoint(j, resolutionindex).getClassification() != 7))
            {
               ...
               switch(colourBy)
               {
                  ...
                  case colourByClassification:
                     ...
                     break;
                  case colourByReturn:
                     rnumber = buckets[i]->getPoint(j, resolutionindex).getReturn();
               ...
               if(heightenNonC || heightenGround || heightenLowVeg || heightenMedVeg || heightenHighVeg || heightenBuildings || heightenNoise || heightenMass || heightenWater || heightenOverlap || heightenUndefined)
               {
                  classification = buckets[i]->getPoint(j, resolutionindex).getClassification();
...
bool TwoDeeOverview::drawbuckets(PointBucket** buckets, int numbuckets)
{
   ...
   // and copy part ways.
   double bucketminx = imageUnitsToPixels(drawnsofarminx - centreSafe.getX()) + (double) (get_width()) / 2 + xoffset;
   double bucketminy = imageUnitsToPixels(drawnsofarminy - centreSafe.getY()) + (double) (get_height()) / 2 + yoffset;
   double bucketmaxx = imageUnitsToPixels(drawnsofarmaxx - centreSafe.getX()) + (double) (get_width()) / 2;
   double bucketmaxy = imageUnitsToPixels(drawnsofarmaxy - centreSafe.getY()) + (double) (get_height()) / 2;
   ...
   if(!drawneverything || (((drawnsofarmaxx - centreSafe.getX() > endx && centre.getX() - centreSafe.getX() > 0) || (drawnsofarminx - centreSafe.getX() < origx && centre.getX() - centreSafe.getX() < 0) || (drawnsofarmaxy - centreSafe.getY() > endy && centre.getY() - centreSafe.getY() > 0) || (drawnsofarminy - centreSafe.getY() < origy && centre.getY() - centreSafe.getY() < 0))))
   {
...
bool TwoDeeOverview::drawviewable(int imagetype)
{
   ...
   //Draw the preview.
   else if(imagetype == 2 || (imagetype == 3 && !drawing_thread->isDrawing() && drawnsinceload))
   {
      //Limits of viewable area:
      double minx = centre.getX() - pixelsToImageUnits(get_width() / 2);
      double maxx = centre.getX() + pixelsToImageUnits(get_width() / 2);
...
void TwoDeeOverview::resetview()
{
   ...
   glLoadIdentity();
   glOrtho(-pixelsToImageUnits(get_width() / 2), +pixelsToImageUnits(get_width() / 2), -pixelsToImageUnits(get_height() / 2), +pixelsToImageUnits(get_height() / 2), -5 * altitude, -5 * depth);
...
bool TwoDeeOverview::pointinfo(double eventx, double eventy)
{
   ...
   if(gotdata)
   {
      ...
      for(unsigned int i = 0; i < pointvector->size(); i++)
      {
         ...
         for(int j = 0; j < (*pointvector)[i]->getNumberOfPoints(0); j++)
         {
            ...
            if(pointsinarea[j])
            {
               ...
               if(!reversez && (*pointvector)[i]->getPoint(j, 0).getZ() >= (*pointvector)[bucketno]->getPoint(pointno, 0).getZ())
               {
                  //Select them.
                  bucketno = i;
                  pointno = j;
      ...
      if(anypoint)
      {
         ...
         pointinfobox->directly_place(topleft, botright, get_width(), get_height());
         draw_pointinfo = true;
   ...
   switch(colourBy)
   {
      case colourByHeight:
         ...
         for(int i = 0; i < 7; ++i)
         {
            ...
            printString(cornx - pixelsToImageUnits(hoffset + hwidth + stringwidth + hgap), corny - padding - pixelsToImageUnits(0.5 * FONT_CHAR_HEIGHT) - length * i, altitude);
         }
         ...
         for(int i = 0; i < 7; ++i)
         {
            ...
            glVertex3d(cornx - pixelsToImageUnits(hoffset + hwidth), corny - padding - i * length, altitude);
            glVertex3d(cornx - pixelsToImageUnits(hoffset), corny - padding - i * length, altitude);
         }
         glEnd();
         break;
      ...
      case colourByIntensity:
         ...
         for(int i = 0; i < 10; ++i)
         {
            ...
            stringwidth = FONT_CHAR_WIDTH * strlen(number);
            glRasterPos3d(cornx - pixelsToImageUnits(hoffset + hwidth + stringwidth + hgap), corny - padding - pixelsToImageUnits(0.5 * FONT_CHAR_HEIGHT) - length * i, altitude);
         }
         // Draws a strip of quads with smooth colour transitions to give a 
         // spectrum-like effect (though the colours are in a different order 
         // from the real spectrum).
         glBegin(GL_QUAD_STRIP);
         glColor3d(1, 1, 1);
         glVertex3d(cornx - pixelsToImageUnits(hoffset + hwidth), corny - padding - 0 * length, altitude);
         glVertex3d(cornx - pixelsToImageUnits(hoffset), corny - padding - 0 * length, altitude);
         glColor3d(0, 0, 0);
         glVertex3d(cornx - pixelsToImageUnits(hoffset + hwidth), corny - padding - 6 * length, altitude);
         glVertex3d(cornx - pixelsToImageUnits(hoffset), corny - padding - 6 * length, altitude);
         glEnd();
         break;
      ...
      case colourByClassification:
         ...
         for(int i = 0; i < 11; ++i)
         {
            ...
            glBegin(GL_QUADS);
            glColor3fv(colour.getRGB());
            glVertex3d(cornx - pixelsToImageUnits(hoffset + hwidth), corny - padding - i * length + pixelsToImageUnits(hwidth / 2), altitude);
            glVertex3d(cornx - pixelsToImageUnits(hoffset), corny - padding - i * length + pixelsToImageUnits(hwidth / 2), altitude);
            glVertex3d(cornx - pixelsToImageUnits(hoffset), corny - padding - i * length - pixelsToImageUnits(hwidth / 2), altitude);
            glVertex3d(cornx - pixelsToImageUnits(hoffset + hwidth), corny - padding - i * length - pixelsToImageUnits(hwidth / 2), altitude);
            glEnd();
         }
...
void TwoDeeOverview::makedistancescale()
{
   ...
   glBegin(GL_LINES);
   //Vertical line.
   glVertex3d(origx + pixelsToImageUnits(50.0), origy + padding, altitude);
   glVertex3d(origx + pixelsToImageUnits(50.0), origy + padding + nummarks * order, altitude);
   //Horizontal lines.
   for(int i = 0; i <= nummarks; ++i)
   {
      glVertex3d(origx + pixelsToImageUnits(50.0), origy + padding + i * order, altitude);
      glVertex3d(origx + pixelsToImageUnits(80.0), origy + padding + i * order, altitude);
   }
   glEnd();
   for(int i = 0; i <= nummarks; ++i)
   {
      ...
      glRasterPos3d(origx + pixelsToImageUnits(85.0), origy + padding + i * order - pixelsToImageUnits(0.5 * FONT_CHAR_HEIGHT), altitude);
   }
}
...
bool TwoDeeOverview::on_ruler_start(GdkEventButton* event)
{
   if(event->button == 1)
   {
      rulerEnd.move(centre.getX() + pixelsToImageUnits(event->x - get_width() / 2), centre.getY() - pixelsToImageUnits(event->y - get_height() / 2), 0);
      rulerStart = rulerEnd;
...
bool TwoDeeOverview::on_zoom(GdkEventScroll* event)
{
   ...
   centre.translate(pixelsToImageUnits(event->x - get_width() / 2), -pixelsToImageUnits(event->y - get_height() / 2), 0);
