...
/*
 ...
 ==================================
 */
LoadWorker::LoadWorker(FileOpener* fo, int point_offset, std::vector<std::string> filenames, bool create_new, bool usearea, int resolutiondepth, int resolutionbase, int bucketlevels, int bucketlimit, int cachelimit, bool default_scale_factors, double scale_factor[3], std::string ascii_code, SelectionBox fence, PointFilter pf, std::string cache_path) :
      Worker(), fileopener(fo), point_offset(point_offset), filenames(filenames), create_new_quadtree(create_new), usearea(usearea), resolutiondepth(resolutiondepth), resolutionbase(resolutionbase), bucketLevels(bucketlevels), bucketlimit(bucketlimit), cachelimit(cachelimit), use_default_scale_factors(default_scale_factors), ascii_code(ascii_code), fence(fence), point_filter(pf), cache_path(cache_path), reader(0)
{
...
Boundary LoadWorker::get_boundary()
{
   ...
   || min_x == 0 || max_x == 0 || min_y == 0 || max_y == 0 || min_y == max_y // no idea why LASlib does this: it just does
   || min_x == max_x)
   {
...
int LoadWorker::load_points(Quadtree* qt)
{
   ...
   int points_to_load = reader->header.number_of_point_records / (point_offset + 1);
   ...
   try
   {
      while(!stopped && reader->read_point())
      {
         ...
         if(usearea)
         {
            if(!vectorTest(fence.getXs(), fence.getYs(), 4, reader->point.get_x(), reader->point.get_y()))
            {
               skip_counter = 0;
               continue;
            }
   ...
   {
      std::cout << "An exception happened in LoadWorker::load_points(): " << e.why() << std::endl;
   }
...
int LoadWorker::load_points_wf(Quadtree* qt)
{
   ...
   try
   {
      ...
      tr1::unordered_map<uint8_t, std::string>::iterator it = point_data_paths.find(flightline_number);
      if(it == point_data_paths.end())
      {
         char fname[] = "/tmp/lag-pointdata.XXXXXX";
         mkstemp(fname);
         point_data_path = std::string(fname);
         point_data_paths.insert(make_pair(flightline_number, point_data_path));
         point_number.resize(flightline_number + 1);
         point_number.at(flightline_number) = 0;
         std::cout << "Creating PointData file at: " << point_data_path << std::endl;
      } else
      {
      ...
      while(!stopped && points_loaded < points_to_load && reader->read_point())
      {
         if(skip_counter != point_offset)
         {
            ++skip_counter;
      ...
      fwrite(point_data_array, points_to_load * sizeof(PointData), 1, point_data_file);
      fclose(point_data_file);
      delete[] point_data_array;
      
   } catch(DescriptiveException& e)
   {
      std::cout << "An exception happend in load_points: " << e.why() << std::endl;
   }
   ...
   try
   {
      ...
      for(int i = 0; i < num_files; ++i)
      {
         ...
         if(filename != "")
         {
            ...
            {
               send_message("Unrecognised file type. Supported formats: las, txt, csv.");
               sig_fail();
               return;
            }
            ...
            if((i == 0 && (create_new_quadtree || !fileopener->get_loadedanyfiles())) || fileopener->get_lidardata() == NULL)
            {
               ...
               if(!usearea)
                  qt = new Quadtree(&boundary, bucketlimit, cachelimit, bucketLevels, resolutionbase, resolutiondepth, cache_path);
               else
               {
                  qt = new Quadtree(fence.get_min_x(), fence.get_min_y(), fence.get_max_x(), fence.get_max_y(), bucketlimit, cachelimit, bucketLevels, resolutionbase, resolutiondepth, cache_path);
               }
               ...
               if(points_loaded == 0)
               {
                  std::cout << "LoadWorker: " << filename << " - no points loaded from file. Possibly because of fence." << std::endl;
               }
...
void LoadWorker::convert_projection()
{
   ...
   if(gpc.get_geo_keys_from_projection(number_of_keys, &geo_keys, num_geo_double_params, &geo_double_params, !gpc.has_projection(false)))
   {
      reader->header.set_geo_keys(number_of_keys, (LASvlr_key_entry*) geo_keys);
      free(geo_keys);
      if(geo_double_params)
      {
         reader->header.set_geo_double_params(num_geo_double_params, geo_double_params);
...
std::string LoadWorker::get_utm_zone()
{
   ...
   if(latlong)
   {
      ...
      gpc.compute_utm_zone((reader->header.min_y + reader->header.max_y) / 2, (reader->header.min_x + reader->header.max_x) / 2, utm);
   ...
   } else
   {
      if(reader->header.vlr_geo_keys)
      {
         ...
         gpc.set_projection_from_geo_keys(reader->header.vlr_geo_keys[0].number_of_keys, (GeoProjectionGeoKeys*) reader->header.vlr_geo_key_entries, reader->header.vlr_geo_ascii_params, reader->header.vlr_geo_double_params);
