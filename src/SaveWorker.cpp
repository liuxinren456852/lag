...
/*
 ...
 ==================================
 */
SaveWorker::SaveWorker(FileSaver* fs, std::string filename, std::string filein, int flightline, std::string parse_string, bool use_latlong, bool use_default_scalefactor, double scale_factor[3], Glib::Mutex* pointbucket_mutex) :
      Worker(), pointbucket_mutex(pointbucket_mutex), filesaver(fs), filename(filename), source_filename(filein), flightline_number(flightline), parse_string(parse_string), latlong_output(use_latlong), latlong_input(false), use_default_scalefactor(use_default_scalefactor), number_of_points(0), reader(0), writer(0), reproject_quantizer(0), saved_quantizer(0)
{
...
void SaveWorker::convert_projection()
{
   ...
   if(latlong_input)
   {
      ...
      gpc.set_utm_projection(const_cast<char*>(zone.str().c_str()), tmp, true);
   ...
   else
   {
      ...
      {
         std::cout << "No projection information in the input file. Will output rubbish.";
      }
   }
   ...
   if(latlong_input && !latlong_output)
   {
      ...
      if(gpc.get_geo_keys_from_projection(number_of_keys, &geo_keys, num_geo_double_params, &geo_double_params, !gpc_ll.has_projection(false)))
      {
