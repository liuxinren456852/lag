...
BoxOverlay::BoxOverlay(Gtk::Label *label, Colour majorc, Colour minorc) :
      directional(false), orthogonalshape(false), slantedshape(true), label(label), majorcolour(majorc), minorcolour(minorc), slantwidth(30), ratio(1), zoomlevel(1)
...
void BoxOverlay::on_start(Point startPoint, double areawidth, double areaheight)
{
   ...
   end.move(centre.getX() + ((startPoint.getX() - areawidth / 2) * ratio / zoomlevel), centre.getY() - ((startPoint.getY() - areaheight / 2) * ratio / zoomlevel), 0);
...
{
   // 0 is set to the centre before scaling and adding the centre coordinate.
   end.move(centre.getX() + ((endPoint.getX() - areawidth / 2) * ratio / zoomlevel), centre.getY() - ((endPoint.getY() - areaheight / 2) * ratio / zoomlevel), 0);
   drawinfo();
}
...
bool BoxOverlay::on_key(GdkEventKey* event, double scrollspeed, bool fractionalshift)
{
   ...
   {
      if(slantedshape)
         scrollspeed *= slantwidth;
      else
         scrollspeed *= theBox.getCorners()[0].distanceTo(theBox.getCorners()[1]);
   }
...
/*
 ...
 ==================================
 */
void BoxOverlay::directly_place(Point start, Point end, double areawidth, double areaheight)
{
...
void BoxOverlay::makeboundaries()
{
   if(slantedshape)
   {
      ...
      theBox = SelectionBox(start.getX() - (slantwidth / 2) * height / length, start.getY() + (slantwidth / 2) * breadth / length, 0.0, start.getX() + (slantwidth / 2) * height / length, start.getY() - (slantwidth / 2) * breadth / length, 0.0, end.getX() + (slantwidth / 2) * height / length, end.getY() - (slantwidth / 2) * breadth / length, 0.0, end.getX() - (slantwidth / 2) * height / length, end.getY() + (slantwidth / 2) * breadth / length, 0.0);
   ...
   } else if(orthogonalshape)
   {
      ...
      if(abs(start.getX() - end.getX()) > abs(start.getY() - end.getY()))
      {
         ...
         theBox = SelectionBox(start.getX(), start.getY(), 0, start.getX(), end.getY(), 0, end.getX(), end.getY(), 0, end.getX(), start.getY(), 0);
      } else
      {
         ...
         // profile.
         theBox = SelectionBox(start.getX(), start.getY(), 0, end.getX(), start.getY(), 0, end.getX(), end.getY(), 0, start.getX(), end.getY(), 0);
      }
   }
}
...
void BoxOverlay::makebox(double rmaxz)
{
   ...
   if(slantedshape)
   {
      ...
      glVertex3d(start.getX() - (slantwidth / 2) * height / length - centre.getX(), start.getY() + (slantwidth / 2) * breadth / length - centre.getY(), altitude);
      ...
      // Near right
      glVertex3d(end.getX() + (slantwidth / 2) * height / length - centre.getX(), end.getY() - (slantwidth / 2) * breadth / length - centre.getY(), altitude);
