...
LagDisplay::LagDisplay(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit) :
      Gtk::GL::DrawingArea(config), zoomlevel(1), zoompower(0.5), lidardata(NULL), bucketlimit(bucketlimit), maindetailmod(0), pointsize(1), ratio(1.0), brightnessBy(brightnessByIntensity), colourBy(colourByFlightline), currentMode(scrolling), cbmaxz(0), cbminz(0), cbmaxintensity(0), cbminintensity(0), zoffset(0), zfloor(0), intensityoffset(0), intensityfloor(0), rmaxz(0), rminz(0), rmaxintensity(0), rminintensity(0), colourheightarray(NULL), brightnessheightarray(NULL), brightnessintensityarray(NULL), red(0.0), green(0.0), blue(0.0), alpha(0.0), drawing_thread(NULL), interrupt_drawing(false), global_pointbucket_mutex(NULL), GL_control_impede(false), GL_data_impede(false)
{
}
...
{
   return Colour(colourheightarray[3 * int(10 * (height - rminz))], colourheightarray[3 * int(10 * (height - rminz)) + 1], colourheightarray[3 * int(10 * (height - rminz)) + 2]);
}
...
/*
 ...
 ==================================
 */
double LagDisplay::brightness_by(double value, double maxvalue, double minvalue, double offsetvalue, double floorvalue)
{
...
/*
 ...
 ==================================
 */
bool LagDisplay::advsubsetproc(vector<PointBucket*>*& pointvector, vector<double> xs, vector<double> ys, int ps)
{
   if(lidardata == NULL)
      return false;
...
void LagDisplay::prepare_image()
{
   ...
   double maxz = (*pointvector)[0]->getmaxZ(), minz = (*pointvector)[0]->getminZ();
   int maxintensity = (*pointvector)[0]->getmaxintensity(), minintensity = (*pointvector)[0]->getminintensity();
...
/*
 ...
 ================================================================================
 */
bool LagDisplay::overlayBox(Colour major, Colour minor, Point corner, Point length, Point breadth)
{
